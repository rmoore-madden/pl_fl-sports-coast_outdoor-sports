...
class Image 
{
	 ...
	 * @param int $offsetY
	 */
	public function cropAndFocus($width, $height, $offsetX = 0, $offsetY = 0): void
	{
		$currentWidth = $this->getWidth();
		$currentHeight = $this->getHeight();

		$offsetX = $this->getOffsetXByPixel($currentWidth, $width, $offsetX);
		$offsetY = $this->getOffsetYByPixel($currentHeight, $height, $offsetY);
		
		$this->_cropImage($width, $height, $offsetX, $offsetY);

		$this->resizeMeta["width"] = $width;
		$this->resizeMeta["height"] = $height;
		$this->resizeMeta["offsetX"] = $offsetX;
		$this->resizeMeta["offsetY"] = $offsetY;
	}
	 ...
	 * @param int $offsetY
	 */
	private function _cropImage($width, $height, $offsetX, $offsetY): void
	{
	 ...
	 * @return float|int
	 */
	private function getOffsetXByPixel($currentWidth, $width, $pixelOffset): int
	{
		$offsetX = 0;
		if ( ($currentWidth > $width) && ($pixelOffset != 0) ) {
			$diff = ($currentWidth - $width);
			$offsetX = ($pixelOffset - ($width / 2));
		}
		return $offsetX;
	}
	 ...
	 * @return float|int
	 */
	private function getOffsetYByPixel($currentHeight, $height, $pixelOffset): int
	{
		$offsetY = 0;
		if ( ($currentHeight > $height) && ($pixelOffset != 0) ) {
			$diff = ($currentHeight - $height);
			$offsetY = ($pixelOffset - ($height / 2));
		}
		return $offsetY;
	}
