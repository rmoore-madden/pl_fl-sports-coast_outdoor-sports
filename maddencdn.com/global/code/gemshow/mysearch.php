...
$query = "SELECT * FROM listings INNER JOIN (
								SELECT listingid, value AS saletype FROM customfields WHERE name LIKE 'Wholesale?%') AS customsaletype
								ON listings.listingid=customsaletype.listingid
						INNER JOIN (
								SELECT listingid, value AS startdate FROM customfields WHERE name LIKE 'Dates Start%') AS customstartdate
								ON listings.listingid=customstartdate.listingid
						INNER JOIN (
								SELECT listingid, value AS enddate FROM customfields WHERE name LIKE 'Date End%') AS customenddate
								ON listings.listingid=customenddate.listingid
						INNER JOIN (
								SELECT listingid, value AS gemshowlocation FROM customfields WHERE name LIKE 'Gem Show Location%') AS customloc
								ON listings.listingid=customloc.listingid
						INNER JOIN (
								SELECT listingid, value AS crossstreets FROM customfields WHERE name LIKE 'Major Cross Streets%') AS customstreet
