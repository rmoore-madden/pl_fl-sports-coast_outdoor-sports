...
(function (root, factory) {
	...
	MMLazyLoad.init = function (options) {
		...
		console.log(settings.loadElements);
		...
		for (var i=0; i < settings.loadElements.length; i++) {
			if ( (IS_BOT) || (settings.loadElements[i].getAttribute("data-load-onload")) ) {
				...
				loadContent(settings.loadElements[i], settings.defaultImage, settings.debug);
			} else {
	...
	function windowScrollListener (e) {
		...
		for (var i=0; i < MMLazyLoadEls.length; i++) {
			...
			if (scrollTop >= (elOffsetTop - offset)) {
				// yes!
				loadContent(MMLazyLoadEls[i], settings.defaultImage, settings.debug);				
	...
	function loadContent (el, defaultImage, debug) {
		...
		console.log(el.getAttribute("data-load-type") + " / " + el.getAttribute("src")  + " / " + el.src);
		console.log(sizeData);
		
		if (sizeData) {
			// and now what type of content is it?
			if ( (el.getAttribute("data-load-type") == "img") && (el.getAttribute("src") == defaultImage) ) {
				// img src
				...
				if (debug) {
					...
					&& (el.getAttribute("poster") == defaultImage) ) {
				// video poster
