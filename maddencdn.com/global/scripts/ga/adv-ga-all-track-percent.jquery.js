/**
 * A COPY OF THIS SCRIPT: http://www.maddenmedia.com/code/adv-ga-all-track-percent.jquery.js
 *	THAT CAN BE SERVED OVER SSL IF NEED BE. OTHERWISE, USE MORE RECENT VERSIONS!
 *
 * Advanced Google Analytics tracking script. Requires Google Analytics and jQuery.
 *	This script will determine which analytics object (universal, legacy, etc.) seems
 *	to be present and will adjust tracking calls based on this.
 */
...
jQuery(function($) {
	...
	var readerLocationPercents = [.0001];
    ...
    function trackLocation () {
        ...
        var height = ($(document).height() - 10); // take 10 off to ensure the can scroll to it
	...
	function sendEvent (eventCategory, eventAction, eventLabel, page) {
		...
		if (typeof ga != "undefined") {
			// universal analytics (updated sept.2k14 for new parameters)
			ga('send', {
				'hitType': 'event',
			...
			if (typeof pageTracker != "undefined") {
				// "traditional syntax"
				pageTracker._trackEvent(eventCategory, eventAction, page, eventLabel);
			} else {
				// "asynchronous syntax"
				_gaq.push(['_trackEvent', eventCategory, eventAction, page, eventLabel]);
				_gaq.push(['pageTracker._trackEvent', eventCategory, eventAction, page, eventLabel]);
			}
		} else if (typeof __gaTracker != "undefined") {
			// wordpress analytics uses a custom tracker object
			// universal analytics (updated sept.2k14 for new parameters)
			__gaTracker('send', {
				'hitType': 'event',
				'eventCategory': eventCategory,
