/**
 * v1.6 - Copyright 2017 Madden Media - All rights reserved.
 *
...
initViewTypes = function () {
	_isMobile = ( (jQuery(DEFAULT_MOBILE_TEST_EL).length != 0) && (jQuery(DEFAULT_MOBILE_TEST_EL).css("float") != "none") ) ? true : false;
	_isTablet = ( (jQuery(DEFAULT_TABLET_TEST_EL).length != 0) && (jQuery(DEFAULT_TABLET_TEST_EL).css("float") != "none") ) ? true : false;
	_doParallax = ( (jQuery(DEFAULT_PARALLAX_TEST_EL).length != 0) && (jQuery(DEFAULT_PARALLAX_TEST_EL).css("float") != "none") ) ? true : false;
}
...
// Later-generation iPads support more desktop-esque funcionality - here's a test for them	
//
// DEPRECATED: use getIsNewerIPad()
//
getIsNewerIpad = function() {
	return getIsNewerIPad();
}
...
adjustMultiSizedImages = function () {
	var msImages = jQuery(_multiSizeImageEl);
	if (msImages.length) {
		msImages.each(function() {
			if ( (! getIsResponsive()) && (jQuery(this).data("src-legacy") != undefined) ) {
				// legacy
				jQuery(this).attr("src", jQuery(this).data("src-legacy"));
			} else if ( (getIsMobile()) && (jQuery(this).data("src-small") != undefined) ) {
				// mobile
				jQuery(this).attr("src", jQuery(this).data("src-small"));
			} else if ( (getIsTablet()) && (jQuery(this).data("src-medium") != undefined) ) {
				// tablet
				jQuery(this).attr("src", jQuery(this).data("src-medium"));
			} else if (jQuery(this).data("src") != undefined) {
				// desktop
